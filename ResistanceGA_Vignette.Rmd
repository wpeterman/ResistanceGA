...
![Transformations](figures/Transformations.svg)
...
```{r}
r.tran<-Resistance.tran(transformation="Monomolecular", shape=2, max=250, r=cont.rf) 

plot.t<-PLOT.trans(PARM=c(2,250), Resistance="C:/ResistanceGA_Examples/SingleSurface/cont.asc", transformation="Monomolecular") 
```
...
Now run the Single surface optimization function (`SS_optim`). Running this example with the default settings
took ~22 minutes to complete on a computer with an Intel i7 3.4 GHz processor.
...
```{r Grid.Search_example, echo=F,cache=TRUE}
...
plot.t<-PLOT.trans(PARM=c(2,250),Resistance="C:/ResistanceGA_Example/cont.asc",transformation="Inverse-Reverse Monomolecular") 
```

Combine raster surfaces together using `Combine_Surfaces`. Note that the .asc files are read in alphabetically. You check the order of surfaces by inspecting `GA.inputs$layer.names`. First, define the parameters that will be passed to `Combine_Surfaces`. Note that the minimum value defined for categorical surfaces is 0. This is because 1 is added to the combined surfaces after they are added together. In this way, the lowest resistance possible on the landscape is 1.  
```{r combine.surfaces}
...
Run `MS_optim`. Running this multisurface example with the default settings took XX minutes to complete on a computer with an Intel i7 3.4 GHz processor.
