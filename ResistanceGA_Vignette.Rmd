...
NOISE <- rnorm(n=length(CS.Resist),mean=0,(0.005*max(CS.Resist)))
...
Note that actual response surfaces tend to be slightly flatter, and the maximum value for a single surface is more difficult to identify precisely. If you were to add some random noise to the CS.response (perhaps more realistic of 'noisy' genetic data), the single surface optimization generally does a good job of recovering the transformation and shape parameters, but the tru maximum value often is not correctly recovered. As you will see below, true maximum values appear to be more accurately recovered when optimizing multiple resistance simultaneously.
