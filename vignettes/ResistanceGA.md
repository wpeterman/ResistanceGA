...
The above optimization can also be done using cost distance calculated in `gdistance`. This approach uses least cost paths between points, so it is a simpler representation of connectivity. However, optimization using `gdistance` is ~35--45% faster than optimization with CIRCUITSCAPE.
...
# Create the true resistance/response surface
gdist.response <- Run_gdistance(gdist.inputs=gdist.inputs,GA.inputs=GA.inputs, r=r.tran)
