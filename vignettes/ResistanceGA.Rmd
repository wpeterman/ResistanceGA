...
vignette: "%\\VignetteIndexEntry{ResistanceGA User Guide} \n%\\VignetteEngine{knitr::rmarkdown}
...
```{r Plot.trans.demo, fig.height = 4, fig.width = 4, fig.align = "center"}
...
```{r single.surface.plot, fig.height = 4, fig.width = 4, fig.align = "center"}
plot(continuous)
...
Gains in optimization speed can be achieved in two ways:
...
```{r feature.sim, warning=FALSE,message=FALSE, fig.height = 4, fig.width = 4, fig.align = "center"}
...
```
Not too bad! As previusly mentioned, the maximum value parameter seems to be the hardest to optimize precisely.

## Bootstrap Analysis
It is often the case that there is weak or ambiguous support for a single best resistance surface (i.e. similar AIC scores). One approach that may lead to additional insight beyond the AIC values generated during optimization is a bootstrap analysis using the `Resist.boot` function. This approach will assess the relative support for each optimized resistance surface through a 'pseudo'bootstrap where the sample locations/individuals as well as the resistance distance matrices are subsampled at each bootstrap iteration (without replacement), and the MLPE model is refit and the AIC scores calculated. Resistance surfaces are *NOT* re-optimized during this process. This analysis will assess the robustness of the optimized resistance surface given different combinations of samples. If the observed patterns in the resistance-genetics relationship are driven by one or a few sample locations, this analysis may reveal this. A full 'analysis' is presented below. Note that for the first time in the simulation of data, 'noise' is being added to the simulated data, making it more akin to empirical genetic data.
```{r boot, eval=FALSE}
data(samples)
data("resistance_surfaces")
...
ecodist::mantel(gd.response ~ gd.true) # Mantel r = 0.66
...
# Run `MS_optim` with all relevatn surfaces
Multi.Surface_optim.gd <- MS_optim(gdist.inputs = gdist.inputs,
                                   GA.inputs = GA.inputs)

# Run `MS_optim` with incorrect random continuous surface
# Modify and replace the continuous surface
set.seed(789)
rand.cont <- resistance_surfaces$continuous
rand.cont <-  rand.rast * rnorm(length(rand.cont), 1, 0.5)
names(rand.cont) <- "random"
pairs(stack(rand.cont, resistance_surfaces$continuous)) # r = 0.19

multi.stack <- stack(resistance_surfaces$categorical,
                     resistance_surfaces$feature,
                     rand.cont)

GA.inputs2 <- GA.prep(method = "LL",
                      ASCII.dir = multi.stack,
...
```
                         surface   avg.weight avg.rank     n Percent.top     k
                           <chr>        <dbl>    <dbl> <dbl>       <dbl> <dbl>
1 categorical.continuous.feature 9.005107e-01    1.044   956        95.6    11
2     categorical.feature.random 9.903130e-02    1.956    44         4.4    11
3                    categorical 4.286153e-04    3.510     0         0.0     4
4                        feature 2.675464e-05    3.561     0         0.0     3
5                     continuous 1.730309e-06    5.011     0         0.0     4
6                       Distance 9.013337e-07    5.918     0         0.0     2
```
The model selection overwhelmingly suggests, correctly, that the multisurface combination with the true continuous surface is the best. Further, if you assess the optimized values from this analysis, you can see that each surface was parameterized very well. The maximum resistance value of all surfaces was lower than the true surface, but overall this resulted in a nearly perfectly correlated surface (r = 0.99). It is worth noting that the optimized surface resulted in a marginal R^2^ = 0.44, suggesting that optimization with 'noisy' genetic data may not result in the highest R^2^ values, but that optimization can nonetheless reliably determine the effects of the landscape on pairwise genetic distance. 
```{r, eval = FALSE}
...
> Summary.table
               Truth Optimized
Category1          1      1.00
Category2        250    217.65
Category3         75     50.59
Transformation     8      8.71
Shape              4      4.00
Max              150    239.94
Feature1           1      1.00
Feature2         350    353.50
