...
```
####__Note: The 'CS.response' object is a simulated response variable for demonstration purposes. *When conducting your own analysis, replace 'CS.response' with the vector containing your pairwise genetic distances*__ &nbsp;   
&nbsp;
...
CS.inputs <- CS.prep(n.Pops = length(sample.locales),
                     ...
                     CS.program = CS.program)
```
With Linux, `CS.program` should be designated as 'csrun.py'
 ...
 * Summary tables for continuous surfaces (ContinuousResults.csv), categorical surfaces (CategoricalResults.csv), and the table of all surfaces (All_Results_Table.csv). This table reports the optimized objective function value, AIC, marginal and conditional R^2^, and log likelihood.   
...
1.  Run the genetics algorithm in parallel (in the `GA.prep` function set `parallel = X` where 'X' is the number of cores to use).
...
Run `GA.prep`. In this example, the genetic algorithm will seek to maximize the log likelihood of the fitted model (`method = "LL"`).
```{r eval=FALSE}
GA.inputs <- GA.prep(ASCII.dir=write.dir,
                     method = "LL"
                     max.cat=500,
